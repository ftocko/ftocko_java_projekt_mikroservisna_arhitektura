...
public class Klijent {
	...
	public Klijent() {
		...
		try {
			...
			this.grupaAdmina = pbp.dajPostavku("sustav.administratori");
	...
	public int registrirajKorisnika(Korisnik noviKorisnik, String korisnik, String zeton) {
	...
	public boolean provjeriAdmina(String korisnik, String zeton) {
		boolean admin = false;
		Client klijent = ClientBuilder.newClient();
		WebTarget webResurs = klijent.target(adresaWA3 + "korisnici").path(korisnik).path("grupe");
		Response restOdgovor = webResurs.request().header("Accept", "application/json").header("korisnik",korisnik)
				.header("zeton",zeton).get();
		List<Grupa> grupeKorisnika = null;
		if (restOdgovor.getStatus() == 200) {
			String odgovor = restOdgovor.readEntity(String.class);
			Gson gson = new Gson();
			grupeKorisnika = new ArrayList<>();
			grupeKorisnika.addAll(Arrays.asList(gson.fromJson(odgovor, Grupa[].class)));
		}
		
		if(grupeKorisnika != null) {
			
			if(grupeKorisnika.size()>0) {
				
				for(Grupa g: grupeKorisnika) {
					if(g.getGrupa().compareTo(grupaAdmina) == 0) {
						admin = true;
						break;
					}
				}
	...
	public int obrisiZetoneKorisnika(String korisnik, String lozinka, String korime) {
		Client klijent = ClientBuilder.newClient();
		WebTarget webResurs = klijent.target(adresaWA3 + "provjere").path("korisnik").path(korime);
		Response restOdgovor = webResurs.request().header("Accept", "application/json").header("korisnik",korisnik)
